...
//      console.log($navDepth1ItemWidth);
	...
	// [PC] nav-depth2-list의 height 구하기
  // function setNavDepth2ListHiehgt(){
  //   var _borderBottomWidth = 2;
  //   var _paddginBottom     = 22;
  //   if($(window).width() < 1260 ) return;
  //   $(window).one('load', function(){
  //     console.log(1);
  //     setTimeout(function(){
  //       var defaultHeight      = $navDepth2List.outerHeight(true) + $nav.height() + _paddginBottom;
  ...
  //       });
  //       $navBG.css({
  //         height : $navDepth2List.outerHeight(true) + _borderBottomWidth,
  //         top    : -1 * ($navDepth2List.outerHeight(true) + _borderBottomWidth),
  //       });
  //     });
  //     clearTimeout();
  //   });
  ...
  function toggleModal(){
    ...
    function closeModal(selector, $opener) {
      ...
      $html.removeClass('is-sroll-blocking');
  ...
  function toggleTab($tab, $tabTarget, $tabButton, tabPanel, tabActiveClass) {
    ...
    return (function(){
      // 페이지 첫 로딩시 탭 높이를 지정
      // var tabButtonHeight = $tabButton.height();
      // var currentTabHeight = $tabTarget.children(tabPanel).outerHeight(true) + tabButtonHeight; 

      // 클릭 이벤트로 활성화 된 탭 높이를 지정
      var setTabHeight = function(){
        $(this).parent().addClass(tabActiveClass).siblings().removeClass(tabActiveClass);
        // var height = $(this).next(tabPanel).outerHeight(true);
        // $tab.height(height + tabButtonHeight);
      };

      // $tab.height(currentTabHeight);
