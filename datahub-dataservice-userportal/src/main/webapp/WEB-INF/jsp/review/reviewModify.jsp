...
<div class="sub__container">
   ...
   <%@ include file="/WEB-INF/jsp/tiles/tiles_breadcrumb.jspf" %>
    ...
    <h3 class="sub__heading1">활용후기 수정<small class="sub__heading1--small">카테고리별 다양한 데이터를 확인하여 보시기 바랍니다.</small></h3>
    <div class="data-write">
      <form id="reviewForm" class="form">
        <input type="hidden" name="reviewOid" value="${param.reviewOid}">
        <fieldset>
                ...
                <tr class="representative-image" style="border-bottom-width:0;">
                  ...
                  <td colspan="3">
                    <div class="representative-image__area" style="padding-bottom:0;">
                      <!-- [D] 이미지가 있을 때 -->
                      <img class="representative-image__thumb-image" :src="download(review.reviewfileOid)" v-if="imageFile.exists" />
                      <!-- [D] 이미지가 없을 때 -->
                      ...
                      <div class="button__group">
                          ...
                          <button class="button button__danger" type="button" v-if="imageFile.exists" @click="deleteImageFile(review.reviewImageFileOid)">삭제</button>
                      </div>
                    </div>
                  </td>
                </tr>
            ...
            <table class="form-table">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td colspan="3">
                      <n2m-file-uploader :saved-files="review.fileList" :only-img="true" @select-file="selectFile" @delete-file="deleteFile"/>
                  </td>
                </tr>
              </tbody>
            </table>
          ...
          <div class="sub__bottom">
            <div class="button__group">
              <a class="button button__primary" href="#none" @click="modifyReview">수정</a>
              <a class="button button__primary" href="#none" @click="deleteReview">삭제</a>
        ...
        methods : {
        	getReview: function(){
                ...
                request.done(function(data){
                    ...
                    vm.review.fileList = vm.review.fileList.map(function(file) {
                    	var result = {};
                    	
                    	if (file.reviewfileFlag === 'file') {
                    		result = {
                   				id: file.reviewfileOid,
                   				name: file.reviewfilePscName,
                   				size: file.reviewfileFileSize
                    		}
                    	} else {
                    		result = {};
                    	}
            ...
            deleteFile: function(index, oid){
            	if (oid) {
            		vm.review.fileList.splice(index, 1);
            ...
            modifyReview : function() {
                ...
                if ((vm.review.fileList.length + vm.fileList.length) <= 0){
                ...
                for(var i=0; i<vm.deleteFileOids.length; i++){
                	reviewFormData.append("deleteFileOids["+i+"]", vm.deleteFileOids[i]);
                }
                ...
                request.done(function(data) {
                    ...
                    location.href = SMC_CTX + "/review/pageDetail.do?reviewOid=${param.reviewOid}";
                });
            ...
            deleteReview: function() {
            	...
            	var request = $.ajax({
                    method : "DELETE",
                    url : SMC_CTX + "/review/remove.do",
                    contentType: "application/json",
//                     dataType: "json",
                    data: JSON.stringify(vm.review)
                });
            ...
            selectImageFile: function(e){
                ...
                if (vm.review.reviewfileOid) {
                	vm.deleteFileOids.push(vm.review.reviewfileOid);
                }
            ...
            deleteImageFile: function(imageFileOid){
            	if (imageFileOid) {
            		vm.deleteFileOids.push(imageFileOid);
            	}
                ...
                switch(code){
                    case 'OPEN_POPUP_REVIEW_DATASETLIST':
                        vm.openReviewDatasetListPopup(function(selectedDatasetList){
                        	if (vm.reviewDatasetList.length === 0) {
                        		vm.reviewDatasetList = selectedDatasetList.map(function(dataset) {
                        			return dataset;
                        		});
                        	} else {
                        		for (var i = 0; i < selectedDatasetList.length; i++) {
                        			var dataset = selectedDatasetList[i];
                        			
                        			if (vm.reviewDatasetList.map(function(data) { return data.id; }).indexOf(dataset.id) === -1) {
                        				vm.reviewDatasetList.push(dataset);
                        			}
                        		}
                        	}
                        });
