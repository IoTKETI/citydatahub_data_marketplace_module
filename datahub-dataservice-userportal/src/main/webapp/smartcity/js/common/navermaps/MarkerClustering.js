...
naver.maps.Util.ClassExtend(MarkerClustering, naver.maps.OverlayView, {
	 ...
	 */
	getOnClickClusterCallback: function() {
		return this.getOptions('onClick');
	},
	 ...
	 */
	setOnClickClusterCallback: function(callback) {
		this.setOptions('onClick', callback);
	},
	/**
  ...
  /**
	...
	initOnClick: function() {
		...
		this._relation = naver.maps.Event.addListener(this._clusterMarker, 'click', naver.maps.Util.bind(function(e) {
			var callback = this._markerClusterer.getOnClickClusterCallback();
			this._markerClusterer.open = !this._markerClusterer.open;
			if(typeof callback === 'function'){
				callback(this._clusterMarker, this._clusterMember);
			}
		}, this));
	},
	...
	updateCluster: function() {
		if (!this._clusterMarker) {
			...
			if ( typeof this._markerClusterer.getOnClickClusterCallback() === 'function') {
				this.initOnClick();
			}
		}
...
//			}
	...
	updateIcon: function() {
		...
		if(this._clusterMember && this._clusterMember.length > 0){
			var UserData           = this._clusterMember[0].UserData;
			var iconContent        = icons[index].content.replace('#title#' , UserData.title).replace('#date#'  , UserData.date);

			var $clusterThis       = this;
			var $cluster           = $(iconContent);
			var $clusterMarker     = $clusterThis._markerClusterer.open ? $cluster.find(".cluster_marker").removeClass("not") : $cluster.find(".cluster_marker").addClass("not");
			var $clusterPopup      = $clusterThis._markerClusterer.open ? $cluster.find(".cluster_popup").removeClass("hidden") : $cluster.find(".cluster_popup").addClass("hidden");
			var $clusterPopupTable = $clusterPopup.find(".pannel__header");
			var $templateRow       = $('<dl><dt class="current-patient__name"></dt><dd class="current-patient__info"></dd></dl>');
			
			for(var i=0, ii=$clusterThis._clusterMember.length; i < ii; i++){
				var marker = $clusterThis._clusterMember[i];
				var $dl = $templateRow.clone();
				$dl.find(".current-patient__name").text(marker.UserData.title);
				$dl.find(".current-patient__info").text(marker.UserData.date);
				$clusterPopupTable.append($dl);
				
				$dl.on('click', function(e){
					var dlIndex = $clusterPopupTable.find('dl').index(this);
					var marker = $clusterThis._clusterMember[dlIndex];
					marker.UserData.vueInstance.statisticsPopupTitle = marker.UserData.title;
					marker.UserData.vueInstance.togglePopup = true;
					var $ul = $("ul.data-write-map").empty();
					marker.UserData.json.split('\n').forEach(function(part){
						var parts = part.split(': ');
						if(parts.length === 2){
							parts[0] = '<span style="color: #c7254e">'+parts[0]+'</span>';
							parts[1] = '<span>'+parts[1]+'</span>';
							$ul.append('<li><p>'+parts.join(': ')+'</p></li>');
						}else{
							$ul.append('<li><p>'+part+'</p></li>');
						}
