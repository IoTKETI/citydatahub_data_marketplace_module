 ...
 <!-- sub-nav -->
    <hr>
      ...
      <div class="sub__container">
                ...
                <table class="form-table">
                  ...
                  <tbody>
                    <tr>
                      ...
                      <td rowspan="2"><button class="button__submit" type="button" @click="getNoticeList(1)">검색</button></td>
                    </tr>
                  </tbody>
                </table>
              </fieldset>
        ...
        <div class="sub__content">
          <h4 class="sub__heading2">공지사항 목록</h4>
          <table class="board-table">
            ...
            <td colspan="4">데이터가 없습니다.</td>
              ...
              <tr v-for="data in list" @click="onRowClick(data)" v-bind:class="[data.noticeFirstFl == 'Y' ? 'first-notice' : '']">
                <th class="text--center" scope="row">{{data.noticeFirstFl == 'N' ? data.noticeId : '우선공지'}}</th>
                <th class="text--left" scope="row">{{data.noticeTitle}}</th>
                <td class="text--center">{{data.noticeCreUsrNm}}</td>
                <td class="text--center">{{data.noticeCreDt|date}}</td>
              </tr>
            </tbody>
          </table>
        </div>
          ...
          data: function(){
            ...
            methods: {
                getNoticeList: function(curPage){
                  ...
                  var request = $.ajax({
                      url: SMC_CTX + "/cusvc/notice/getList.do",
                      method: "GET",
                      dataType: "json",
                      data: {
                ...
                onRowClick : function(row){
                       location.href = SMC_CTX + "/cusvc/notice/pageDetail.do?noticeOid="+row.noticeOid;
                },
